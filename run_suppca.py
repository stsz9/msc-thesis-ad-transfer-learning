...
if __name__ == "__main__":
    ...
    print("\nSupPCA: Framing 2 (alone)", flush=True)
    run_supervised_benchmark(Xg, yg, method="supervised_pca", n_components=10, n_screen=2000,
                             framing="alone", num_trials=30, name="SupPCA alone")
    if have_cov:
        print("\nSupPCA: Framing 1 (+covariates+APOE)", flush=True)
        Xcomb = np.hstack([Xg, Xcov]); n_geno = Xg.shape[1]
        run_supervised_benchmark(Xcomb, yg, method="supervised_pca", n_components=10, n_screen=2000,
                                 framing="covariates", n_geno=n_geno, num_trials=30, name="SupPCA +cov")
